...
non_covid_expression_data = non_covid_expression_data[target_genes,]
...
covid_icu_expression_data = fread(file_path, sep='\t', header = TRUE, data.table = FALSE, select = covid_icu_patients)
row.names(covid_icu_expression_data) <- genes
covid_icu_expression_data = covid_icu_expression_data[target_genes,]
